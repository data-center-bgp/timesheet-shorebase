...
- Migrations are plain SQL files under `supabase/migrations/`, applied with the Supabase CLI directly against the self-hosted Postgres connection string — this project does not use `supabase link` (that's for supabase.com-hosted projects). Run: `npx supabase db push --db-url "<DATABASE_URL from .env.local>"`.
