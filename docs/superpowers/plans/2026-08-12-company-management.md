...
- **Type consistency:** `Company` (id/name/internal/start_date/end_date) defined once in `CompanyForm.tsx` and imported by the edit page (Task 4); `CompanyRow` in the list page (Task 2) is a separately-declared but structurally identical type. This is deliberate, not an oversight: Task 2 (list page) runs before Task 3 (which creates `CompanyForm.tsx`, the only place `Company` exists), so the list page cannot import a type that doesn't exist yet at that point in the plan's sequence. A type-only import would in fact be safe once `CompanyForm.tsx` exists (type imports are erased at compile time and never pull a Client Component's runtime code across the server/client boundary) — this is not the same hazard as the `SIDEBAR_COOKIE` bug elsewhere in this codebase, which involved importing a runtime *value* from a `'use client'` module. The two five-field types are kept separate here only because of task ordering, not a technical requirement. `createCompany`'s signature `(prevState, formData)` and `updateCompany`'s `(id, prevState, formData)` both match how `CompanyForm`'s `action` prop is typed and how `.bind(null, id)` is used in the edit page. `deactivateCompany`/`reactivateCompany` take only `id`, matching the existing `signOut` Server Action's zero-argument-beyond-bind pattern already in `src/app/actions.ts`, and are assignable to `<form action>` because TypeScript allows a function with fewer parameters to satisfy a callback type expecting more.
